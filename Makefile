...
IMAGE_MUSICFOX_NAMES := $(shell $(TAGS_SCRIPT) musicfox "$(TAG_VERSION)" "$(REGISTRIES)")
...
IMAGE_MUSICFOX_TAGS := $(foreach ARCH,$(SUBIMAGES),$(foreach IMAGE,$(IMAGE_MUSICFOX_NAMES), $(IMAGE)-$(ARCH)))
...
.PHONY: musicfox-%
musicfox-%:
	@echo "building $(IMAGE_MUSICFOX_NAME)-$*"
	docker build --platform=linux/$* \
...
.PHONY: goreleaser
musicfox: $(patsubst %, musicfox-%,$(SUBIMAGES))
...
.PHONY: docker-push-musicfox
docker-push-musicfox:
	@$(foreach IMAGE, $(IMAGE_MUSICFOX_TAGS), docker push $(IMAGE);)
